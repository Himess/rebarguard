...
const AGENT_LABELS: Record<AgentMessage['agent'], { name: string; color: string; icon: string }> = {
  plan_parser: { name: 'Plan Parser', color: '#38bdf8', icon: '📐' },
  geometry: { name: 'Geometry Agent', color: '#a78bfa', icon: '📏' },
  code: { name: 'Code Compliance', color: '#f472b6', icon: '📜' },
  fraud: { name: 'Fraud Agent', color: '#ef4444', icon: '🕵️' },
  risk: { name: 'Seismic Risk', color: '#f97316', icon: '⚠️' },
  material: { name: 'Material Agent', color: '#10b981', icon: '🔩' },
  cover: { name: 'Cover Agent', color: '#60a5fa', icon: '📐' },
  moderator: { name: 'Moderator', color: '#f59e0b', icon: '⚖️' },
};
            ...
            >
              ...
              {m.evidence && (
                <details className="mt-2 text-xs text-[var(--color-text-muted)]">
                  <summary className="cursor-pointer hover:text-white">Evidence (JSON)</summary>
